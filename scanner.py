...
def generate_markdown(results: list, market: dict) -> str:
    ...
    alerts = [r for r in results if r["status"].startswith(("🔴", "🟠"))]
    cautions = [r for r in results if r["status"].startswith("🟡")]
    healthy = [r for r in results if r["status"].startswith("🟢")]

    # Alerts first (action needed)
    if alerts:
        ...
        for r in alerts:
            ...
            lines.append(f"|--------|-------|")
            lines.append(f"| LTP | ₹{r['ltp']} |")
            lines.append(f"| SMA 100 | ₹{r['sma100']} |")
            lines.append(f"| SMA 350 | ₹{r['sma350']} |")
            lines.append(f"| RSI(14) | {r['rsi']} |")
            lines.append(f"| ADX(14) | {r['adx']} |")
            lines.append(f"| +DI / -DI | {r['plus_di']} / {r['minus_di']} |")
            lines.append(f"| Extension | {r['extension_pct']}% |")
            lines.append("")
            for a in r["alerts"]:
                lines.append(f"> ⚠️ {a}")
            lines.append("")
